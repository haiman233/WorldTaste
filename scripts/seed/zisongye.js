...
var GrowTimems_INFINITE = 150000; // 无尽植物生长周期间隔，单位毫秒
...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_ZISONGYE") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_ZISONGYE");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_ZISONGYE") {
        let dropItem = (itemId) => {
        ...
        if (Infinite_Yes_1 < 1) { // 100%概率掉落
            dropItem("WT_ZISONGYE");
            dropItem("WT_SEED_ZISONGYE");
        }
    }
}
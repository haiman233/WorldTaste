...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_CULI") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_CULI");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_CULI") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_CULI", probability: 0.7 },
            { itemId: "WT_HEICULI", probability: 0.2 },
            { itemId: "WT_LANCULI", probability: 0.1 }
        ];

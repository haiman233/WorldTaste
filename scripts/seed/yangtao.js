...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_YANGTAO") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_YANGTAO");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_YANGTAO") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_YANGTAO", probability: 0.85 },
            { itemId: "WT_QINGYANGTAO", probability: 0.15 },
            { itemId: "WT_SEED_YANGTAO", probability: 0.2 },
        ];

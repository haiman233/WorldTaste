...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_HONGYUANLUOBO") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_HONGYUANLUOBO");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_HONGYUANLUOBO") {
        let dropItem = (itemId) => {
        ...
        if (Infinite_Yes_1 < 1) { // 100%概率掉落
            dropItem("WT_HONGYUANLUOBO");
            dropItem("WT_SEED_HONGYUANLUOBO");
        }
    }
}
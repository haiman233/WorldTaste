...
var steps = [1/3, 1/3, 1/3, 1/3, 1/3, 1/3, 2/3]; // 生长阶段
...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_QINGGANLAN") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_QINGGANLAN");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_QINGGANLAN") {
        let dropItem = (itemId) => {
        ...
        if (Infinite_Yes_1 < 1) { // 100%概率掉落
            dropItem("WT_QINGGANLAN");
            dropItem("WT_SEED_QINGGANLAN");
        }
    }
}
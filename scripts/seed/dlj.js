...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_DLJ") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_DLJ");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_DLJ") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_DLJ", probability: 0.7 },
            { itemId: "WT_CDLJ", probability: 0.2 },
            { itemId: "WT_LDLJ", probability: 0.1 },
            { itemId: "WT_SEED_DLJ", probability: 0.3 }
        ];

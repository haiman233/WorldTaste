...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_TONGNIAN") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_TONGNIAN");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_TONGNIAN") {
        let dropItem = (itemId) => {
        ...
        if (Infinite_Yes_1 < 0.2) { // 100%概率掉落
            dropItem("WT_SEED_TONGNIAN");
        }
        let Infinite_Yes_2 = Math.random();
        if (Infinite_Yes_2 < 0.2) { // 100%概率掉落
            dropItem("WT_WANDOUSHESHOU");
        }
        let Infinite_Yes_3 = Math.random();
        if (Infinite_Yes_3 < 0.12) { // 100%概率掉落
            dropItem("WT_DAZUIHUA");
        }
        let Infinite_Yes_4 = Math.random();
        if (Infinite_Yes_4 < 0.1) { // 100%概率掉落
            dropItem("WT_HUIMIE");
        }
        if (Infinite_Yes_4 < 0.5) { // 100%概率掉落
            dropItem("WT_XIANGRIKUI");
        }
        let Infinite_Yes_5 = Math.random();
        if (Infinite_Yes_5 < 0.08) { // 100%概率掉落
            dropItem("WT_MALIAOMOGU");
        }
    }
}
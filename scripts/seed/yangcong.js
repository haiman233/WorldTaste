...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_YANGCONG") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_YANGCONG");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_YANGCONG") {
        let dropItem = (itemId) => {
        ...
        if (Infinite_Yes_1 < 1) { // 100%概率掉落
            dropItem("WT_HUANGPIYANGCONG");
            dropItem("WT_SEED_YANGCONG");
        }
    }
}
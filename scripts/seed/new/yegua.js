...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_YEGUA") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_YEGUA");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_YEGUA") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_QINGPITIANGUA", probability: 0.6 },
            { itemId: "WT_JINSIGUA", probability: 0.4 },
            { itemId: "WT_XINGHONGTIANGUA", probability: 0.1 },
            { itemId: "WT_MABOGUA", probability: 0.4 },
            { itemId: "WT_YINGGUANGLVGUA", probability: 0.2 },
            { itemId: "WT_SEED_YEGUA", probability: 0.3 }
        ];

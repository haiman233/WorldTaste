...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_GANHAN") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_GANHAN");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_GANHAN") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_DUOROUZHIWU", probability: 0.5 },
            { itemId: "WT_DWANGHUA", probability: 0.35 },
            { itemId: "WT_HUOBANGXRZ", probability: 0.3 },
            { itemId: "WT_BAIHUAXRZ", probability: 0.2 },
            { itemId: "WT_YOUNIXRZ", probability: 0.3 },
            { itemId: "WT_ZUANSHIXRZ", probability: 0.1 },
            { itemId: "WT_TLRYXRZ", probability: 0.15 },
            { itemId: "WT_SEED_GANHAN", probability: 0.2 }
        ];

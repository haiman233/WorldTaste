...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_DOUJIA") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_DOUJIA");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_DOUJIA") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_HONGDOU", probability: 0.5 },
            { itemId: "WT_LVDOU", probability: 0.6 },
            { itemId: "WT_HEIDOU", probability: 0.4 },
            { itemId: "WT_HUANGDOU", probability: 0.5 },
            { itemId: "WT_LANDOU", probability: 0.1 },
            { itemId: "WT_SEED_DOUJIA", probability: 0.3 }
        ];

...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_XINGHONG") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_XINGHONG");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_XINGHONG") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_XUETENG:", probability: 0.5 },
            { itemId: "WT_NAOXINCAI", probability: 0.4 },
            { itemId: "WT_SADANGUO", probability: 0.3 },
            { itemId: "WT_SEED_XINGHONG", probability: 0.3 }
        ];

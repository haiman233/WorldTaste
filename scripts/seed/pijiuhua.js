...
var GrowTimems_INFINITE = 180000; // 无尽植物生长周期间隔，单位毫秒
var steps = [0, 0, 0, 0, 0, 0, 1/3]; // 生长阶段
...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_PIJIUHUA") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_PIJIUHUA");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_PIJIUHUA") {
        let dropItem = (itemId) => {
        ...
        if (Infinite_Yes_1 < 1) { // 100%概率掉落
            dropItem("WT_PIJIUHUA");
            dropItem("WT_SEED_PIJIUHUA");
        }
    }
}
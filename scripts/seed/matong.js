...
var GrowTimems_INFINITE = 210000; // 无尽植物生长周期间隔，单位毫秒
...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_MATONG") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_MATONG");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_MATONG") {
        let dropItem = (itemId) => {

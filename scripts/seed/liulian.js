...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_LIULIAN") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_LIULIAN");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_LIULIAN") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_LIULIAN", probability: 0.9 },
            { itemId: "WT_QINGNILIULIAN", probability: 0.1 },
            { itemId: "WT_SEED_LIULIAN", probability: 0.2 }
        ];

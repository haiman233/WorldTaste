...
function tick(info) {
    ...
    // 处理无尽植物生长逻辑
    if (machinesf === "WT_SEED_NANGUA2") {
        handleGrowth(world, location, lastUseTime, currentTime, "WT_SEED_NANGUA2");
    }
}
...
function handleHarvest(world, location) {
    ...
    if (sfItem.getId() === "WT_SEED_NANGUA2") {
        // 定义所有可能掉落的物品及其掉落概率
        let drops = [
            { itemId: "WT_NANGUA2", probability: 0.4 },
            { itemId: "WT_LANGUA", probability: 0.2 },
